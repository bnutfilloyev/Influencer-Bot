...
async def change_lang(call: types.CallbackQuery):
    ...
    users.update_one({'user_id': call.from_user.id}, {'$set': {'lang': lang}}, upsert=True)
    if users.find_one({'user_id': call.from_user.id}).get('phone'):
        await call.message.answer(texts[lang]['phone_number_button'], reply_markup=await phone_number_keyboard(lang))
        return
